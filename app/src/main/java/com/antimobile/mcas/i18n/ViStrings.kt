...
object ViStrings : AppStrings {
    ...
    private object Settings : SettingsStrings {
        ...
        override val mmsAutoDownloadTitle = "Tự động tải MMS"
        override val mmsAutoDownloadSubtitle = "Tải ảnh MMS khi SIM không chuyển vùng; luôn có thể tải thủ công trong hội thoại."
        override val mmsRoamingTitle = "Tải MMS khi chuyển vùng"
        override val mmsRoamingSubtitle = "Có thể phát sinh phí dữ liệu chuyển vùng từ nhà mạng."
    ...
    private object OutgoingCall : OutgoingCallStrings {
        ...
        override val roleGateBody =
            "Android yêu cầu bạn chọn MCAS làm ứng dụng chuyển tiếp cuộc gọi. Quyền này chỉ giúp MCAS nhận số và SIM trước khi gọi để hiển thị cảnh báo; MCAS luôn giữ nguyên số, không chuyển hướng và không chặn cuộc gọi đi."
        override val roleGateAction = "Chọn MCAS để nhận biết cuộc gọi"
        override val roleUnavailableTitle = "Thiết bị chưa hỗ trợ"
        override val roleUnavailableBody =
            "Thiết bị này không cung cấp vai trò chuyển tiếp cuộc gọi của Android nên MCAS chưa thể nhận biết cuộc gọi đi."
        ...
        override val roleExplanation =
            "Android cấp vai trò xử lý cuộc gọi đi để MCAS nhận số và SIM trước khi gọi. MCAS luôn giữ nguyên số, không chuyển hướng và không chặn cuộc gọi đi."
        ...
        override val overlayPermissionSubtitle =
            "Nếu chưa cấp quyền, MCAS sẽ tự dùng thông báo heads-up làm phương án dự phòng."
        ...
        override val privacyNote =
            "Số gọi và thông tin nhà mạng chỉ được xử lý trên thiết bị. MCAS không chuyển hướng, thay đổi hay tự thực hiện cuộc gọi."
    ...
    private object Legal : LegalStrings {
        override fun lastUpdated(date: String) = "Cập nhật lần cuối: $date · Áp dụng cho ứng dụng MCAS"
    ...
    private object Permission : PermissionStrings {
        ...
        override val callLogDesc =
            "MCAS đọc nhật ký cuộc gọi trên máy để hiển thị lại đầy đủ: loại cuộc (đi / đến / nhỡ), " +
        ...
        override val simDesc =
            "MCAS đọc nhà mạng của SIM đang lắp (SIM 1 / SIM 2) để xác định mỗi cuộc gọi là nội mạng " +
        ...
        override val contactsDesc =
            "Android chỉ chuyển cuộc gọi từ số đã lưu tới MCAS khi quyền Danh bạ còn hiệu lực. " +
                "MCAS dùng quyền này để áp dụng đầy đủ quy tắc chặn và hiển thị tên, ảnh; ứng dụng " +
        ...
        override val smsStep = "Nhắn tin SMS"
        override val smsHeadline = "Nhắn tin ngay trong MCAS"
        override val smsDesc =
            "Chọn MCAS làm ứng dụng SMS mặc định để đọc hội thoại, gửi và nhận SMS/MMS ngay trong " +
                "ứng dụng. Android sẽ luôn hỏi bạn xác nhận và bạn có thể đổi lại bất cứ lúc nào trong Cài đặt."
        override val smsBullet1 = "Đọc, gửi và nhận SMS/MMS theo thao tác của bạn"
        override val smsBullet2 = "Gửi bằng đúng SIM bạn chọn trên máy hai SIM"
        override val smsBullet3 = "Chỉ dùng quyền nhắn tin sau khi MCAS là ứng dụng SMS mặc định"
        override val smsRoleDeniedMessage =
            "Bạn chưa chọn MCAS làm ứng dụng SMS mặc định. Hãy xác nhận trong màn hình của Android để tiếp tục."
        ...
        override val consentIntro =
            "Bước cuối cùng trước khi bắt đầu. Vui lòng đọc nhanh vài điểm quan trọng rồi đồng ý với " +
                "điều khoản để sử dụng MCAS."
        override val consentPoint1 = "Nhật ký cuộc gọi và Danh bạ luôn chỉ đọc. Khi bạn chủ động đặt MCAS làm ứng dụng SMS mặc định, MCAS sẽ đọc, tạo, gửi, nhận, cập nhật hoặc xoá SMS theo thao tác của bạn."
        ...
        override val consentCheckLabel = "Tôi đã đọc và đồng ý với Điều khoản sử dụng và Chính sách quyền riêng tư của MCAS."
    ...
    private object Actions : ActionStrings {
        ...
        override val feedbackEmailSubject = "Góp ý ứng dụng MCAS"
    ...
    private object Donate : DonateStrings {
        ...
        override val heroMessage =
            "MCAS được làm bằng tất cả tâm huyết và luôn miễn phí. Nếu ứng dụng có ích cho bạn, một ly cà phê nhỏ sẽ tiếp thêm động lực để mình duy trì và cải thiện. Hoàn toàn tự nguyện — không ủng hộ, mọi tính năng vẫn nguyên vẹn."
        ...
        override val shareSubject = "Mã QR ủng hộ MCAS"
        ...
        override val footerMessage =
            "Đây là khoản đóng góp tự nguyện dành cho nhà phát triển, không phải phí bắt buộc và không đổi lấy bất kỳ tính năng nào. MCAS vẫn luôn miễn phí. Xin chân thành cảm ơn sự ủng hộ của bạn."
    ...
    private object Blocker : CallBlockStrings {
        ...
        override val featureInfoAvailabilityNote =
            "Chỉ áp dụng cho cuộc gọi đến khi MCAS đang là ứng dụng sàng lọc, Bảo vệ cuộc gọi đang bật và không trong thời gian tạm dừng."
        override val roleTitle = "Bật chặn cuộc gọi"
        override val roleBody =
            "Android yêu cầu bạn chọn MCAS làm ứng dụng sàng lọc cuộc gọi. MCAS chỉ sàng lọc cuộc gọi đến, không thay ứng dụng gọi điện mặc định của máy."
        override val roleAction = "Chọn MCAS để chặn spam"
        override val roleUnavailableTitle = "Thiết bị chưa hỗ trợ"
        override val roleUnavailableBody = "Thiết bị này không cung cấp vai trò sàng lọc cuộc gọi của Android."
        override val roleActive = "MCAS đang là ứng dụng sàng lọc cuộc gọi"
        ...
        override val dailyScheduleDescription =
            "Lặp lại vào các thứ đã chọn. Trong khung Chặn, MCAS bật bảo vệ; trong khung Ngưng, mọi cuộc gọi được cho qua."
        ...
        override val blockMethodSubtitle =
            "Chọn cách MCAS xử lý khi cuộc gọi khớp quy tắc hoặc bị cổng bảo vệ số lạ giữ lại trước ngưỡng."
        ...
        override val notificationSubtitle = "Bật hoặc tắt cảnh báo âm thanh mỗi khi MCAS chặn cuộc gọi."
        ...
        override val alwaysAllowDetails =
            "Thêm số bằng cách nhập tay, chọn từ Danh bạ, Lịch sử cuộc gọi hoặc Phân loại nhóm. Khi một số trong danh sách này gọi đến, MCAS cho qua ngay.\n\n" +
        ...
        override val blockedNumbersDetails =
            "Thêm số bằng cách nhập tay, chọn từ Danh bạ, Lịch sử cuộc gọi hoặc Phân loại nhóm. Khi một số trong danh sách này gọi đến, MCAS áp dụng phương thức chặn bạn đã chọn.\n\n" +
        ...
        override val groupBlockingDetails =
            "Chọn cách MCAS xử lý số đã lưu trong danh bạ và số ngoài danh bạ. Mỗi lựa chọn đều ghi rõ cuộc gọi sẽ được cho qua, bị chặn hay tiếp tục xét theo Quy tắc nâng cao.\n\n" +
        ...
        override val advancedRulesDetails =
            "Dùng bộ lọc cuộc gọi có dấu hiệu spam hoặc tạo điều kiện theo đầu số, đuôi số, chuỗi số, độ dài, nhà mạng, khu vực hay loại cuộc gọi. Mỗi quy tắc có thể áp dụng cho số trong danh bạ, ngoài danh bạ hoặc tất cả.\n\n" +
                "MCAS xét các quy tắc đang bật từ trên xuống và áp dụng quy tắc đầu tiên khớp."
        ...
        override val unknownPolicyPassDesc = "MCAS vẫn xét Danh sách chặn và Quy tắc nâng cao. Nếu không có gì khớp, cuộc gọi được cho qua."
        ...
        override val unknownPolicyBlockUntilRepeatDesc = "Nếu không khớp quy tắc nào, MCAS chặn các lần gọi đầu và cho qua khi số đó gọi lặp đạt ngưỡng trong thời gian đã đặt."
        ...
        override val ruleActionLabel = "MCAS sẽ làm gì?"
        ...
        override val processingGuideItemTitle = "Tìm hiểu cách MCAS chặn cuộc gọi"
        override val processingGuideItemSubtitle = "Xem thứ tự MCAS kiểm tra và xử lý một cuộc gọi đến."
        override val processingGuideSheetTitle = "Cách MCAS xử lý cuộc gọi"
        override val processingGuideIntro = "MCAS kiểm tra lần lượt theo thứ tự dưới đây và dừng ngay khi đã có kết quả."
        ...
        override fun processingGuideStepDescription(step: Int) = when (step) {
            ...
            4 -> "MCAS áp dụng lựa chọn dành cho số trong hoặc ngoài danh bạ. Chọn Áp dụng theo quy tắc để tiếp tục bước kế tiếp."
            5 -> "MCAS xét từ trên xuống. Quy tắc đầu tiên khớp sẽ được áp dụng."
        ...
        override fun commonIssueCause(issue: Int) = when (issue) {
            1 -> "MCAS có thể không còn là ứng dụng sàng lọc cuộc gọi; Bảo vệ cuộc gọi đang tắt, tạm dừng hoặc nằm trong khung giờ tạm dừng. Sau khi buộc dừng ứng dụng, hoặc trước lần mở khóa đầu tiên sau khi khởi động lại, Android cũng có thể chưa chuyển cuộc gọi cho MCAS."
            ...
            4 -> "Thông báo có thể đang tắt trong MCAS, bị tắt ở khung giờ hiện tại, chưa được Android cấp quyền, hoặc kênh thông báo đã bị tắt. Chỉ cuộc gọi thực sự bị chặn mới tạo thông báo; Chỉ tắt tiếng và Bỏ qua không tạo cảnh báo chặn."
            ...
            6 -> "Lịch sử của MCAS chỉ ghi cuộc gọi thực sự bị chặn. Cuộc gọi được cho qua hoặc chỉ tắt tiếng không được ghi tại đây; Lịch sử chặn cũng không phải là bản sao Nhật ký cuộc gọi của Android."
            else -> "Android chuẩn thường chỉ chuyển cho ứng dụng sàng lọc những cuộc gọi có số điện thoại hiển thị hợp lệ. Số ẩn/không khả dụng và cuộc gọi VoIP/SIP phụ thuộc ứng dụng Điện thoại cùng phần mở rộng của từng hãng máy nên có thể không tới MCAS."
        }
        override fun commonIssueFix(issue: Int) = when (issue) {
            1 -> "Mở lại MCAS, cấp lại quyền chặn cuộc gọi và bật Bảo vệ cuộc gọi. Kiểm tra lịch bật/tạm dừng; sau khi khởi động lại hãy mở khóa thiết bị ít nhất một lần."
            ...
            4 -> "Chọn Thông báo mỗi lần chặn, kiểm tra lịch thông báo nâng cao, rồi cho phép thông báo và bật kênh cảnh báo của MCAS trong Android."
            ...
            else -> "Không thể bảo đảm chặn các cuộc gọi mà Android không chuyển cho MCAS. Với số có hiển thị, hãy thêm số đó vào Danh sách chặn; với số ẩn hoặc VoIP, kiểm tra thêm tính năng chặn của ứng dụng Điện thoại và nhà mạng."
        }
        ...
        override val emptyHistory = "Chưa có cuộc gọi nào bị chặn. Lịch sử sẽ xuất hiện tại đây sau khi MCAS chặn."
        ...
        override val spamRiskPickerDescription =
            "Cuộc gọi có ít nhất một dấu hiệu rủi ro do MCAS nhận diện."
        override val spamRiskDetailsTitle =
            "MCAS chặn khi phát hiện ít nhất một dấu hiệu sau:"
        ...
        override val spamRiskUnknownPrefixDetail =
            "Số di động Việt Nam có 10 chữ số nhưng đầu số chưa được MCAS nhận diện."
        override val spamRiskVerificationDetail =
            "Trên Android 11 trở lên, mạng di động báo không xác minh được số gọi. Nếu thiết bị không cung cấp thông tin này, MCAS bỏ qua dấu hiệu đó."
        ...
        override val voipExplanation =
            "VoIP (Voice over Internet Protocol) là công nghệ truyền giọng nói qua mạng IP như Wi-Fi, 4G/5G hoặc Internet thay vì chỉ dùng mạng thoại truyền thống.\n\n" +
                ...
                "Trong MCAS, mục “Cuộc gọi Internet (quy tắc cũ)” chỉ được giữ để đọc dữ liệu cũ. Khi tạo quy tắc mới, hãy chọn điều kiện SIP phù hợp nếu thiết bị cung cấp URI cuộc gọi."
        override val sipExplanation =
            "SIP (Session Initiation Protocol) là giao thức dùng để thiết lập, quản lý và kết thúc phiên gọi qua mạng IP. Một định danh SIP thường có dạng sip:user@domain; sips: là biến thể dùng kết nối bảo mật cho tín hiệu SIP.\n\n" +
                "Phần user trước dấu @ có thể là số điện thoại, ví dụ sip:+84912345678@provider.vn, hoặc chuỗi chữ như sip:support@company.vn. MCAS tách hai trường hợp này thành hai điều kiện cuộc gọi đặc biệt.\n\n" +
                "Android chuẩn thường chỉ chuyển địa chỉ tel: tới CallScreeningService. MCAS chỉ nhận diện được SIP/sips khi thiết bị hoặc nhà sản xuất chuyển URI đó cho ứng dụng."
        override val uriExplanation =
            "URI (Uniform Resource Identifier) là chuỗi dùng để định danh một tài nguyên. Trong cuộc gọi, URI cho biết kiểu địa chỉ và giá trị của bên gọi.\n\n" +
                ...
                "MCAS chỉ phân loại điều kiện SIP khi URI hợp lệ, có scheme sip: hoặc sips: và có phần user trước dấu @."
        override val cliExplanation =
            "CLI (Calling Line Identification) là thông tin số điện thoại của bên gọi mà mạng hoặc dịch vụ cuộc gọi cung cấp, ví dụ +84912345678. MCAS dùng CLI cho các quy tắc theo số, đầu số, đuôi số, độ dài, nhà mạng hoặc khu vực.\n\n" +
        ...
        override val specialAndroidLimit =
            "Android chuẩn chỉ chuyển handle tel: vào CallScreeningService. SIP/sips là khả năng mở rộng tùy thiết bị hoặc OEM (Original Equipment Manufacturer — nhà sản xuất thiết bị); nếu URI không được chuyển tới MCAS thì ứng dụng không có dữ liệu để nhận diện tiêu chí SIP."
        ...
        override val contactPickerPermissionBody =
            "Cho phép MCAS đọc danh bạ để bạn chọn liên hệ và nhận diện số lạ. Ứng dụng không sửa hoặc gửi danh bạ đi."
        ...
        override val notificationChannelDescription =
            "Cảnh báo heads-up, âm thanh và rung khi MCAS vừa chặn một cuộc gọi"
    ...
    private object Backup : BackupStrings {
        ...
        override val callLogNote =
            "Nhật ký cuộc gọi hệ thống là chỉ đọc nên không nằm trong bản sao lưu; riêng lịch sử do MCAS đã chặn có thể sao lưu tại đây."
        ...
        override val secBlockHistorySub = "Các sự kiện MCAS đã chặn"
        ...
        override val errInvalidFile = "File không hợp lệ hoặc không phải bản sao lưu của MCAS."
    ...
    private object Messaging : MessagingStrings {
        ...
        override val unsupportedBody = "MCAS không tìm thấy phần cứng nhắn tin di động trên thiết bị này."
        override val roleTitle = "Đặt MCAS làm ứng dụng SMS"
        override val roleBody =
            "Để hiển thị, gửi và nhận tin nhắn ngay trong MCAS, Android cần bạn chọn MCAS làm ứng dụng SMS mặc định. Bạn có thể đổi lại bất cứ lúc nào trong Cài đặt."
        override val setDefault = "Đặt làm mặc định"
        override val permissionTitle = "Cho phép nhắn tin"
        override val permissionBody =
            "MCAS cần quyền SMS và thông tin SIM để đọc hội thoại, nhận tin đến và gửi đúng SIM bạn chọn."
        override val grantPermissions = "Cấp quyền nhắn tin"
        override val roleRequired = "MCAS không còn là ứng dụng SMS mặc định."
        ...
        override val emptyBody = "Tin nhắn SMS và MMS của bạn sẽ xuất hiện ở đây."
        ...
        override val deleteConversationBody = "Toàn bộ SMS và MMS trong hội thoại này sẽ bị xoá khỏi thiết bị. Không thể hoàn tác."
        override val delete = "Xoá"
        ...
        override val messageHint = "Tin nhắn"
        ...
        override val attachPhoto = "Đính kèm ảnh"
        override val removeAttachment = "Bỏ ảnh"
        override val subjectHint = "Tiêu đề MMS (không bắt buộc)"
        override val mmsLabel = "MMS"
        override val downloadMms = "Tải MMS"
        override val downloadingMms = "Đang tải MMS…"
        override val mmsDownloadFailed = "Không tải được MMS. Chạm để thử lại."
        override val imageProcessing = "Đang tối ưu ảnh cho MMS…"
        override val mmsConfirmTitle = "Gửi dưới dạng MMS?"
        override val mmsConfirmBody = "Ảnh và tiêu đề được gửi qua MMS bằng dữ liệu di động của SIM đã chọn. Nhà mạng có thể tính phí MMS."
        override val sendMms = "Gửi MMS"
        override val imageUnreadable = "Không thể đọc ảnh này. Hãy chọn ảnh khác."
        override val imageTooLarge = "Ảnh vẫn vượt giới hạn MMS của nhà mạng sau khi tối ưu."
        override val subjectTooLong = "Tiêu đề MMS dài hơn giới hạn của nhà mạng. Hãy rút ngắn tiêu đề."
        override val mmsDisabled = "Nhà mạng hoặc SIM đã tắt MMS."
        override val mobileDataDisabled = "Hãy bật dữ liệu di động để gửi hoặc tải MMS."
        override val openImage = "Mở ảnh"
        override val notificationChannelName = "Tin nhắn"
        override val notificationChannelDescription = "Thông báo SMS mới nhận trong MCAS"
        ...
        override val directReplyFailed = "Không thể gửi câu trả lời. Mở MCAS để kiểm tra SIM."
    }
}
