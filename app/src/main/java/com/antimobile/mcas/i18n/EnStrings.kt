...
object EnStrings : AppStrings {
    ...
    private object Settings : SettingsStrings {
        ...
        override val mmsAutoDownloadTitle = "Auto-download MMS"
        override val mmsAutoDownloadSubtitle = "Download MMS photos when the SIM isn't roaming; manual download remains available."
        override val mmsRoamingTitle = "Download MMS while roaming"
        override val mmsRoamingSubtitle = "Your carrier may charge roaming data fees."
    ...
    private object OutgoingCall : OutgoingCallStrings {
        ...
        override val roleGateBody =
            "Android needs you to choose MCAS as the call-redirection app. This access only lets MCAS receive the number and SIM before dialing so it can show an alert; MCAS always keeps the number unchanged and never redirects or blocks outgoing calls."
        override val roleGateAction = "Choose MCAS for call detection"
        override val roleUnavailableTitle = "Not supported on this device"
        override val roleUnavailableBody =
            "This device does not provide Android's call-redirection role, so MCAS cannot detect outgoing calls."
        ...
        override val roleExplanation =
            "Android grants an outgoing-call role so MCAS receives the number and selected SIM before dialing. MCAS always keeps the number unchanged and never blocks outgoing calls."
        ...
        override val overlayPermissionSubtitle =
            "Until it is granted, MCAS falls back to a heads-up notification."
        ...
        override val privacyNote =
            "The number and carrier information are processed only on your device. MCAS never redirects, changes, or places the call."
    ...
    private object Legal : LegalStrings {
        override fun lastUpdated(date: String) = "Last updated: $date · Applies to the MCAS app"
    ...
    private object Permission : PermissionStrings {
        ...
        override val callLogDesc =
            "MCAS reads the call log on your device to show it in full: call type (outgoing / incoming / missed), " +
        ...
        override val simDesc =
            "MCAS reads the carrier of the inserted SIMs (SIM 1 / SIM 2) to tell whether each call is on-net " +
        ...
        override val contactsDesc =
            "Android only sends calls from saved numbers to MCAS while Contacts permission remains granted. " +
                "MCAS uses it to apply every blocking rule and show names and photos; it only reads contacts " +
        ...
        override val smsStep = "SMS messaging"
        override val smsHeadline = "Message inside MCAS"
        override val smsDesc =
            "Choose MCAS as your default SMS app to read conversations and send or receive SMS/MMS in one " +
                "place. Android always asks for confirmation, and you can change the default again in Settings."
        override val smsBullet1 = "Read, send and receive SMS/MMS when you ask"
        override val smsBullet2 = "Send with the SIM you choose on dual-SIM devices"
        override val smsBullet3 = "Use messaging access only while MCAS is your default SMS app"
        override val smsRoleDeniedMessage =
            "MCAS has not been selected as the default SMS app. Confirm the choice on Android's screen to continue."
        ...
        override val consentIntro =
            "One last step before you start. Please quickly read a few key points, then accept the terms to use MCAS."
        override val consentPoint1 = "Call history and Contacts always remain read-only. If you explicitly make MCAS your default SMS app, it can read, create, send, receive, update or delete SMS messages as you direct."
        ...
        override val consentCheckLabel = "I have read and agree to MCAS's Terms of Use and Privacy Policy."
    ...
    private object Actions : ActionStrings {
        ...
        override val feedbackEmailSubject = "MCAS app feedback"
    ...
    private object Donate : DonateStrings {
        ...
        override val heroMessage =
            "MCAS is made with care and stays free forever. If the app helps you, a small coffee keeps me motivated to maintain and improve it. Entirely voluntary — every feature stays the same whether you donate or not."
        ...
        override val shareSubject = "MCAS support QR code"
        ...
        override val footerMessage =
            "This is a voluntary contribution to the developer — not a required fee, and it doesn't unlock any features. MCAS stays free for everyone. Thank you so much for your support."
    ...
    private object Blocker : CallBlockStrings {
        ...
        override val featureInfoAvailabilityNote =
            "Applies only to incoming calls while MCAS is the active call-screening app and Call protection is on and not paused."
        override val roleTitle = "Enable call blocking"
        override val roleBody =
            "Android needs you to choose MCAS as the call-screening app. MCAS only screens incoming calls; it does not replace your phone's default dialer."
        override val roleAction = "Choose MCAS for spam blocking"
        override val roleUnavailableTitle = "Not supported on this device"
        override val roleUnavailableBody = "This device does not provide Android's call-screening role."
        override val roleActive = "MCAS is the current call-screening app"
        ...
        override fun repeatCallerExceptionSubtitle(threshold: Int, minutes: Int) =
            "This feature only applies when MCAS confirms the number is outside Contacts and the call matches no rule. Attempts before threshold $threshold use the selected Handling method: the two Block modes record blocked-call history and notify according to settings; Silence only mutes the ringtone and creates no blocked-call record; Allow passes every call and does not count attempts. Once there are $threshold attempts in the most recent $minutes-minute window, the current and later calls are allowed while that rolling window remains at the threshold. If the number already matches a rule, the repeat feature is not evaluated and never overrides the result. The saved-contacts exception still applies separately to broad rules. Only while evaluating this repeated-caller feature, if MCAS cannot verify Contacts, it allows the call to avoid a false block."
        ...
        override val blockMethodSubtitle =
            "Choose what MCAS does when a call matches a rule or is held below the unknown-caller gate threshold."
        ...
        override val alwaysAllowDetails =
            "Add a number manually or pick it from Contacts, call history, or Categories. When a number in this list calls, MCAS allows it immediately.\n\n" +
        ...
        override val blockedNumbersDetails =
            "Add a number manually or pick it from Contacts, call history, or Categories. When a number in this list calls, MCAS uses your selected blocking method.\n\n" +
        ...
        override val groupBlockingDetails =
            "Choose how MCAS handles numbers saved in Contacts and numbers outside Contacts. Each choice clearly says whether the call is allowed, blocked, or continues to Advanced rules.\n\n" +
        ...
        override val advancedRulesDetails =
            "Use the spam-risk signals filter or create conditions for prefixes, suffixes, contained digits, length, carrier, region, or call type. Each rule can apply to saved numbers, unsaved numbers, or everyone.\n\n" +
                "MCAS checks enabled rules from top to bottom and applies the first matching rule."
        ...
        override val unknownPolicyPassDesc = "MCAS still checks the Block list and Advanced rules. If nothing matches, the call is allowed."
        ...
        override val unknownPolicyBlockUntilRepeatDesc = "If no rule matches, MCAS blocks the first calls and allows the number after it reaches the repeat threshold within the selected time."
        ...
        override val ruleActionLabel = "What should MCAS do?"
        ...
        override val processingGuideItemTitle = "Learn how MCAS handles calls"
        override val processingGuideItemSubtitle = "See the order MCAS uses to check and handle an incoming call."
        override val processingGuideSheetTitle = "How MCAS handles a call"
        override val processingGuideIntro = "MCAS checks the following steps in order and stops as soon as it has a result."
        ...
        override fun processingGuideStepDescription(step: Int) = when (step) {
            ...
            4 -> "MCAS applies the selected choice for saved or unsaved numbers. Choose Apply rules to continue."
            5 -> "MCAS checks from top to bottom. The first matching rule is applied."
        ...
        override fun commonIssueCause(issue: Int) = when (issue) {
            1 -> "MCAS may no longer be the call-screening app, or Call protection may be off, paused, or inside a scheduled pause. Android may also stop sending calls to MCAS after the app is force-stopped or before the first unlock following a restart."
            ...
            4 -> "Notifications may be off in MCAS, disabled for the current scheduled period, denied by Android, or disabled for the notification channel. Only a call that is actually blocked creates an alert; Silence only and Allow through do not create blocked-call alerts."
            ...
            6 -> "MCAS history records only calls that were actually blocked. Allowed and silence-only calls are not recorded there, and blocked-call history is not a copy of Android's system call log."
            else -> "Standard Android generally sends a screening app only calls with a valid, visible telephone number. Private or unavailable callers and VoIP/SIP calls depend on the Phone app and device-maker extensions, so they may never reach MCAS."
        }
        override fun commonIssueFix(issue: Int) = when (issue) {
            1 -> "Reopen MCAS, grant the call-blocking role again, and turn on Call protection. Check enabled/pause schedules; after a restart, unlock the device at least once."
            ...
            4 -> "Select Notify on every block, check the advanced notification schedule, then allow notifications and enable the MCAS alert channel in Android."
            ...
            else -> "MCAS cannot guarantee blocking when Android does not deliver the call for screening. Add a visible number to the Block list; for private or VoIP calls, also check blocking features from the Phone app and your carrier."
        }
        ...
        override val emptyHistory = "No calls have been blocked yet. MCAS will show them here after blocking."
        ...
        override val spamRiskPickerDescription =
            "A call with at least one risk signal recognized by MCAS."
        override val spamRiskDetailsTitle =
            "MCAS blocks when it detects at least one of these signals:"
        ...
        override val spamRiskUnknownPrefixDetail =
            "A 10-digit Vietnamese mobile number has a prefix MCAS does not recognize."
        override val spamRiskVerificationDetail =
            "On Android 11 or later, the mobile network cannot verify the caller's number. MCAS skips this signal if the device provides no verification information."
        ...
        override val voipExplanation =
            "VoIP (Voice over Internet Protocol) carries voice over an IP network such as Wi-Fi, 4G/5G, or the Internet instead of relying only on a traditional voice network.\n\n" +
                ...
                "In MCAS, “Internet call (legacy rule)” is retained only for reading older data. For a new rule, choose the appropriate SIP condition when the device supplies the call URI."
        override val sipExplanation =
            "SIP (Session Initiation Protocol) is used to establish, manage, and end calling sessions over an IP network. A SIP identity commonly looks like sip:user@domain; sips: uses a secured connection for SIP signaling.\n\n" +
                "The user part before @ may be a phone number, such as sip:+84912345678@provider.vn, or text such as sip:support@company.vn. MCAS exposes these as two separate special-call conditions.\n\n" +
                "Standard Android normally delivers only tel: addresses to CallScreeningService. MCAS can identify SIP/sips only when the device or manufacturer passes that URI to the app."
        override val uriExplanation =
            "URI (Uniform Resource Identifier) is a string that identifies a resource. For a call, the URI describes the kind of caller address and its value.\n\n" +
                ...
                "MCAS classifies a SIP condition only when the URI is valid, uses the sip: or sips: scheme, and contains a user before @."
        override val cliExplanation =
            "CLI (Calling Line Identification) is the caller's phone-number information supplied by the network or calling service, for example +84912345678. MCAS uses CLI for rules based on a number, prefix, suffix, length, carrier, or region.\n\n" +
        ...
        override val specialAndroidLimit =
            "Standard Android sends only tel: handles to CallScreeningService. SIP/sips support is an OEM (Original Equipment Manufacturer) or device extension; if the URI is not delivered to MCAS, the app has no data with which to identify the SIP criterion."
        ...
        override val contactPickerPermissionBody =
            "Allow MCAS to read contacts so you can choose people and identify unknown numbers. The app never edits or uploads your contacts."
        ...
        override val notificationChannelDescription =
            "Heads-up, sound and vibration when MCAS blocks a call"
    ...
    private object Backup : BackupStrings {
        ...
        override val callLogNote =
            "The system call log is read-only and is not backed up; MCAS's own blocked-call history can be backed up here."
        ...
        override val secBlockHistorySub = "Events blocked by MCAS"
        ...
        override val errInvalidFile = "Invalid file, or not a MCAS backup."
    ...
    private object Messaging : MessagingStrings {
        ...
        override val unsupportedBody = "MCAS couldn't find cellular messaging hardware on this device."
        override val roleTitle = "Make MCAS your SMS app"
        override val roleBody =
            "To show, send and receive messages inside MCAS, Android needs you to choose MCAS as the default SMS app. You can change it again in Settings at any time."
        override val setDefault = "Set as default"
        override val permissionTitle = "Allow messaging"
        override val permissionBody =
            "MCAS needs SMS and SIM access to read conversations, receive messages and send with the SIM you choose."
        override val grantPermissions = "Grant messaging access"
        override val roleRequired = "MCAS is no longer the default SMS app."
        ...
        override val emptyBody = "Your SMS and MMS conversations will appear here."
        ...
        override val deleteConversationBody = "Every SMS and MMS in this conversation will be deleted from the device. This can't be undone."
        override val delete = "Delete"
        ...
        override val removeAttachment = "Remove photo"
        override val subjectHint = "MMS subject (optional)"
        override val mmsLabel = "MMS"
        override val downloadMms = "Download MMS"
        override val downloadingMms = "Downloading MMS…"
        override val mmsDownloadFailed = "MMS couldn't be downloaded. Tap to retry."
        override val imageProcessing = "Optimizing photo for MMS…"
        override val mmsConfirmTitle = "Send as MMS?"
        override val mmsConfirmBody = "Photos and subjects use MMS over mobile data on the selected SIM. Your carrier may charge for MMS."
        override val sendMms = "Send MMS"
        override val imageUnreadable = "This photo couldn't be read. Choose another photo."
        override val imageTooLarge = "The photo still exceeds your carrier's MMS limit after optimization."
        override val subjectTooLong = "The MMS subject exceeds your carrier's limit. Shorten the subject."
        override val mmsDisabled = "MMS is disabled by the carrier or SIM."
        override val mobileDataDisabled = "Turn on mobile data to send or download MMS."
        override val openImage = "Open photo"
        override val notificationChannelName = "Messages"
        override val notificationChannelDescription = "New SMS notifications received by MCAS"
        ...
        override val directReplyFailed = "The reply couldn't be sent. Open MCAS to check the SIM."
    }
